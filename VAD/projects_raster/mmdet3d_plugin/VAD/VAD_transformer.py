...
class VADPerceptionTransformer(BaseModule):
    ...
    def init_weights(self):
        ...
        #         if 'channel_adapter' in name:
        #             print(f"BEFORE init_weights - {name}: mean={param.data.mean():.6f}, std={param.data.std():.6f}")

        # for name, p in self.named_parameters():
        #     if 'SDRaster_encoder' in name or 'satellite_encoder' in name or 'raster_encoder' in name:
        #         print(f"跳过初始化: {name}")
        #         continue
                
        #     if p.dim() > 1:
        #         print(f"初始化: {name}, shape={p.shape}")
        #         nn.init.xavier_uniform_(p)

        # if hasattr(self, 'satellite_encoder') and self.satellite_encoder is not None:
        #     print("=== after init_weights ===")
        #     for name, param in self.satellite_encoder.named_parameters():
        #         if 'channel_adapter' in name:
        #             print(f"AFTER init_weights - {name}: mean={param.data.mean():.6f}, std={param.data.std():.6f}")
    ...
    def forward(self,
        ...
        if self.raster_encoder is not None and (satellite_img is not None or SDRaster_img is not None):  
            prior_bev = self.raster_encoder(
                ...
                sd=SDRaster_img,           # [1, 200, 200, 3] or None  
            )
        else:
            prior_bev = None

        if prior_bev is not None and self.fusion_module is not None:
            bev_embed = self.fusion_module(bev_embed, prior_bev)
        else:
            bev_embed = bev_embed
