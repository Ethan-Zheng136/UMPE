...
class AlphaScheduleHook(Hook):
    def __init__(self, 
                 start_iter=0,
                 end_iter=20000,
                 start_alpha=0.2,      # 从0.2开始
                 target_alpha=0.6,     # 目标是0.6
                 schedule_type='cosine',
                 update_mode='smooth'):  # 新增：'direct' 或 'smooth'
        """
        强制控制 fusion_module 的 Alpha 余弦调度 Hook：0.2 → 0.6
        """
        ...
        self.momentum = 0.1 if update_mode == 'smooth' else 1.0  # 平滑过渡的动量
    ...
    def get_current_alpha_target(self, current_iter):
        ...
        if current_iter < self.start_iter:
            ...
            # 从 start_alpha 线性插值到 target_alpha
...
class FusionAlphaMonitorHook(Hook):
    """监控 fusion_module alpha，记录到 log_buffer，供 TensorBoard 可视化"""
    ...
    def after_train_iter(self, runner):
        ...
        for name, param in runner.model.named_parameters():
            if 'fusion_module.alpha' in name:
                ...
                # 写到 log_buffer，TensorBoard 会自动收集
                runner.log_buffer.output['fusion_alpha/value'] = alpha_val

                # 额外安全检查
                if alpha_val < -0.01:
                    runner.logger.warning(f'⚠️ Negative fusion alpha: {alpha_val:.6f}')
                if alpha_val > 0.25:
                    runner.logger.warning(f'⚠️ Large fusion alpha: {alpha_val:.6f}')
                break

        # 如果 AlphaScheduleHook 已经写了 max_alpha_allowed，这里还可以顺便记录利用率
