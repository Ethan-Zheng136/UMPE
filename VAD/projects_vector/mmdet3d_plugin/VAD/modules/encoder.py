...
class MapGraphTransformer(BaseModule):
    ...
    def init_weights(self):
        # 对map_embedding进行Xavier初始化
        xavier_init(self.map_embedding, distribution='uniform')
        
        # 对TransformerEncoder的各层进行初始化
    ...
    def process_map_data(self, prior_map, map_type="sdmap", img_feats=None):
        ...
        # 获取设备信息（从输入张量中推断）
        # device = next(iter(prior_map.values()))[0].device if prior_map else torch.device('cpu')
        # dtype = next(iter(prior_map.values()))[0].dtype if prior_map else torch.float32
        device = img_feats[0].device
        dtype = torch.float32
        
        num_categories = max(TYPE_VOCAB.values()) + 1
        # 使用 torch.tensor 替代 np.array
        source_code = torch.tensor([1, 0], dtype=dtype, device=device) if map_type == 'sdmap' else torch.tensor([0, 1], dtype=dtype, device=device)

        map_graph_list = []      # 存储多边形张量的列表
        onehot_category_list = []  # 存储类别 one-hot 编码的列表
        onehot_source_list = []    # 存储来源 one-hot 编码的列表
    ...
    def forward(self, sdmap, map_type, img_feats):
        ...
        for graph_polylines, onehot_cat, onehot_src in zip(map_graph, onehot_category, onehot_source):
            ...
            # print(graph_polylines.shape, onehot_cat.shape, onehot_src.shape)
            # print(graph_polylines.shape)
            # if onehot_cat.shape[1] == 0:
            #     graph_feat = graph_polylines.view(npolylines, npoints * pdim)
            # if len(onehot_category) == 0:
                # graph_feat = torch.cat([graph_polylines.view(npolylines, npoints * pdim), torch.zeros(), onehot_source], dim=-1)
            ...
            # map_graph_shapes = torch.tensor([mfeat.shape[0] for mfeat in batch_graph_feats], 
            #                                     device=batch_graph_feats[0].device)
            ...
            #                                         mfeat.shape[1])], dim=0) for mfeat in batch_graph_feats
            # ])
        # return map_graph_feats, confidence_token
...
class SineContinuousPositionalEncoding(BaseModule):
    ...
    def init_weights(self):
        # 这个模块没有需要学习的参数，所以不需要额外的初始化
        # 但如果有任何可学习的参数，可以在这里初始化
        pass
...
class AlignNet(BaseModule):
    """
    将矢量化的HDMap和SDMap进行微对齐的AlignNet模块
    
    输入:
        bev_embed: BEV特征嵌入 [B, C, H, W] 或 [B, npolylines, 11, C]
        sd_token: SDMap token特征 [B, npolylines, 11, C]
        sdmap: SDMap点云坐标 [B, npolylines, 11, 2]
    
    输出:
        sdmap_aligned: 对齐后的SDMap点云 [B, npolylines, 11, 2]
        transform_params: 变换参数字典，包含theta和t
    """
    
    def __init__(self, 
                 hidden_dim=256,
                 in_channels=256,  # bev_embed和sd_token拼接后的通道数
                 ):
        super().__init__()
        ...
        self.mlp = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(hidden_dim // 2, 3)  # 输出theta, tx, ty
        )
    ...
    def forward(self, bev_embed, sd_token, sdmap, map_type, img_feats):
        ...
        # 处理旋转角度θ (使用sigmoid限制在[0,1]范围，然后映射到[-π, π])
        ...
        # 应用变换到SDMap点云
        # 首先将sdmap重塑为 [B, npolylines * 11, 2]
        sdmap_flat = sdmap[0].reshape(B, -1, 2)  # [B, npolylines * 11, 2]
        
        # 应用旋转和平移: S_aligned = R · S^T + t
        # 注意: 我们需要将点云转置为 [B, 2, npolylines * 11] 以便进行矩阵乘法
...
class DualVecCrossFusion(BaseModule):
    """
    HD/SD 双路跨注意力（各自用 log(U) 偏置）+ presence 归一门控 + 零初始化残差融合 + 训练态 Source Dropout。
    目前不做 TopK / TokenDrop ；未来加法见注释。
    """
    def __init__(self,
                 embed_dims=256,
                 num_heads=8,
        ...
        # 门控：全局可学习 logits（最稳健），对缺席来源做 presence 归一
        ...
        #     nn.Conv2d(64, 512, 1),               # 64 -> 512 (256*2)
        # )

        # 残差融合：零初始化，alpha 从 0 学起
        # self.ln = nn.LayerNorm(embed_dims)
        # self.proj = nn.Linear(2 * embed_dims, embed_dims)
        # self.alpha = nn.Parameter(torch.tensor(0.0))

        # 训练态 Source Dropout：两路都在时以 p 随机丢一路
    ...
    # def forward(self,
    #             bev_tok,                     # [B,Nq,C]
    #             hd_tokens=None,   # [B,Nh,C], [B,Nh]
    #             sd_tokens=None,):  # [B,Ns,C], [B,Ns]
        ...
        if use_hd:
            ...
            # y_hd = self.ca_hd(bev_tok, hd_tokens)  # [B,Nq,C]
        if use_sd:
            y_sd = self.ca_sd(bev_tok, sd_tokens, sd_U)  # [B,Nq,C]
            # y_sd = self.ca_sd(bev_tok, sd_tokens)  # [B,Nq,C]

        # presence 归一门控（缺席来源自动分到 0）
        pres = torch.tensor([float(use_hd), float(use_sd)], device=bev_tok.device)  # [2]
        # formed_y_hd = y_hd.permute(0, 2, 1).view(B, C, self.bev_h, self.bev_w) # [1, 20000, 256] -> [1, 256, 200, 100]
        # formed_y_sd = y_sd.permute(0, 2, 1).view(B, C, self.bev_h, self.bev_w)

        # concat_tk = torch.cat([formed_y_hd, formed_y_sd], dim=1)     # [B, 512, 200, 100]
        
        # weights = self.gate_conv(concat_tk)                 # [B, 512, 1, 1]
        ...
        # pres = pres.unsqueeze(0).unsqueeze(0)                # [B, 1, 2]
        # g = normalized_weights * pres                        # [B, 256, 2]
        # g = g / g.sum(dim=2, keepdim=True).clamp_min(1e-6)   # [B, 256, 2]
        # hd_weights = g[:, :, 0].view(B, 1, C)                # [B, 1, 256]
        # sd_weights = g[:, :, 1].view(B, 1, C)                # [B, 1, 256]

        # fused = hd_weights * y_hd + sd_weights * y_sd             # [B,Nq,C]
        # if use_hd and use_sd:
        #     prior_mix = 0.5 * y_hd + 0.5 * y_sd
        # elif use_hd:
        #     prior_mix = y_hd
        # else:
        #     prior_mix = y_sd
        ...
        # fused = bev_tok + self.alpha * self.proj(torch.cat([self.ln(bev_tok), self.ln(prior_mix)], dim=-1))
