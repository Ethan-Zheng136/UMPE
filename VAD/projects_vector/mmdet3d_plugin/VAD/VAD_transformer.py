...
class VADPerceptionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        if fusion_layer is not None:
            self.fusion_layer = build_transformer_layer_sequence(fusion_layer)
        else:
    ...
    def get_bev_features(
        ...
        if prior_tokens is not None:
            bev_embed, fusion_info = self.fusion_layer(bev_embed, hd_tokens=prior_tokens[0], hd_U=prior_tokens[2], sd_tokens=prior_tokens[1], sd_U=prior_tokens[3])
