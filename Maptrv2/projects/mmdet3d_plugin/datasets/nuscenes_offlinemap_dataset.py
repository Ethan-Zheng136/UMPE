...
class CustomNuScenesOfflineLocalMapDataset(CustomNuScenesDataset):
    ...
    def get_data_info(self, index):
        ...
        input_dict = dict(
            ...
            # ===zgt===
            # satellite_filename = info['satellite_filename'],
            index = index
        )
        input_dict['sdmap'] = self.vectorized_sdmap[info['token']]
        input_dict['hdmap'] = self.vectorized_hdmap[info['token']]
        # ===zgt===
        # print('index:', index)
        # satellite_base_path = "/DATA_EDS2/zhenggt2407/MapTR-maptrv2/data/nuscenes/satellite_map_dataset/rotated_trainval"
        # input_dict["satellite_filename"] = os.path.join(satellite_base_path, input_dict['satellite_filename'])
        # print("satellite_filename:", input_dict["satellite_filename"])
