...
class MapTRPerceptionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        if raster_encoder is not None:  
            ...
            self.raster_encoder = None     
        if fusion_module is not None:
            self.fusion_module = build_transformer_layer_sequence(fusion_module)
        else:
            self.fusion_module = None
        if vector_fusion_layer is not None:
            self.vector_fusion_layer = build_transformer_layer_sequence(vector_fusion_layer)
        else:
            self.vector_fusion_layer = None
    ...
    def init_weights(self):
        ...
        for name, p in self.named_parameters():
            if 'raster_encoder' in name:
                print(f"Jump over init: {name}")
                continue
                
            if p.dim() > 1:
                print(f"Init: {name}, shape={p.shape}")
    ...
    def unit_norm(self, x, eps=1e-6):
        # print(f"[unit_norm] output={x.device}")
        y = x / (torch.norm(x, dim=-1, keepdim=True).clamp_min(eps))
        # print(f"[unit_norm] output={y.device}")
        return y
    ...
    def forward(self,
        ...
        # if self.vector_fusion_layer is not None and vector_feats is not None:
        #     # print('vector_feats hd:', vector_feats[0])
        #     bev_embed, _ = self.vector_fusion_layer(
        #         bev_embed, 
        ...
        # return bev_embed, depth, inter_states, init_reference_out, inter_references_out, results_for_seloss, T_params
