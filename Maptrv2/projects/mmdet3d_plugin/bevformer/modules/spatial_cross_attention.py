...
@ATTENTION.register_module()
class CrossAttentionBEV(BaseModule):
    def __init__(self,
                 embed_dims=256,
                 num_heads=8,
                 dropout=0.1,
                 ...
                 epsilon=1e-6):
        """
        BEV特征与地图折线特征的交叉注意力模块
        
        Args:
            embed_dims (int): 特征维度
            num_heads (int): 注意力头数
            dropout (float): Dropout概率
            qkv_bias (bool): 是否在线性层添加偏置
            init_cfg (dict): MMCV初始化配置
        """
        ...
        # 验证维度可被头数整除
        if self.head_dim * num_heads != self.embed_dims:
    ...
    def forward(self, bev_embed, prior_feats, confidence_token):
    # def forward(self, bev_embed, prior_feats):
        """
        Args:
            bev_embed (Tensor): BEV特征 [batch_size, bev_h*bev_w, embed_dims]
            prior_feats (Tensor): 地图折线特征 [batch_size, num_polylines, embed_dims]
        
        Returns:
            Tensor: 注意力输出 [batch_size, bev_h*bev_w, embed_dims]
        """
        batch_size, seq_len_q, _ = bev_embed.shape
        _, seq_len_kv, _ = prior_feats.shape
        
        # 线性投影
        q = self.q_proj(bev_embed)  # [B, H*W, D]
        k = self.k_proj(prior_feats)  # [B, N, D]
        v = self.v_proj(prior_feats)  # [B, N, D]
        
        # 多头切分 [B, seq_len, num_heads, head_dim]
        ...
        # 转置为 [B, num_heads, seq_len, head_dim]
        q = q.permute(0, 2, 1, 3)
        k = k.permute(0, 2, 1, 3)
        v = v.permute(0, 2, 1, 3)
        
        # 计算注意力分数 [B, num_heads, H*W, N]
        attn_scores = torch.matmul(q, k.transpose(-2, -1)) / (self.head_dim ** 0.5)


        # # 使用置信度生成偏置 - 将置信度转换为log空间并扩展维度
        # # 添加epsilon防止log(0)的情况
        confidence_bias = torch.log(confidence_token.clamp(min=self.epsilon))
        
        # # 扩展维度以匹配注意力分数 [B, num_heads, H*W, N]
        confidence_bias = confidence_bias.unsqueeze(1).unsqueeze(1)  # [B, 1, 1, N]
        confidence_bias = confidence_bias.expand(-1, self.num_heads, seq_len_q, -1)  # [B, num_heads, H*W, N]
        
        # # 将置信度偏置添加到注意力分数中
        attn_scores = attn_scores + confidence_bias
        ...
        # 注意力输出 [B, num_heads, H*W, head_dim]
        attn_output = torch.matmul(attn_weights, v)
        
        # 合并多头输出 [B, H*W, D]
        attn_output = attn_output.permute(0, 2, 1, 3).contiguous()
        attn_output = attn_output.view(batch_size, seq_len_q, self.embed_dims)
    ...
    def __repr__(self):
        ...
        s += f'(embed_dims={self.embed_dims}, '
