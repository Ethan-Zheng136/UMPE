...
@TRANSFORMER_LAYER.register_module()
class MyCustomBaseTransformerLayer2(BaseModule):
    """Base `TransformerLayer` for vision transformer.
    It can be built from `mmcv.ConfigDict` and support more flexible
    customization, for example, using any number of `FFN or LN ` and
    use different kinds of `attention` by specifying a list of `ConfigDict`
    ...
    def __init__(self,
        ...
        super(MyCustomBaseTransformerLayer2, self).__init__(init_cfg)

        self.batch_first = batch_first

        num_attn = operation_order.count('self_attn') + operation_order.count(  ## sdmap
            'cross_attn') + operation_order.count('cross_attn_bev') + operation_order.count('cross_attn_graph')  ## sdmap
        if isinstance(attn_cfgs, dict):
            attn_cfgs = [copy.deepcopy(attn_cfgs) for _ in range(num_attn)]
        else:
            assert num_attn == len(attn_cfgs), f'The length ' \
        ...
        index = 0
        for operation_name in operation_order:
            if operation_name in ['self_attn', 'cross_attn', 'cross_attn_bev', 'cross_attn_graph']:  ## sdmap
                if 'batch_first' in attn_cfgs[index]:
                    assert self.batch_first == attn_cfgs[index]['batch_first']
                else:
                    attn_cfgs[index]['batch_first'] = self.batch_first
